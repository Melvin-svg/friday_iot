...
export const ArduinoWorkspace: React.FC<ArduinoWorkspaceProps> = ({ code, fileName, onSendMsg, setCode }) => {
  ...
  return (
    <div className="flex flex-col gap-5 h-full">
      {/* Box 1: C++ Sketch */}
      <div className="glass-panel flex flex-col h-[370px] overflow-hidden flex-shrink-0">
        <div className="flex items-center justify-between border-b border-white/5 bg-slate-950/20 px-4 py-2 flex-shrink-0">
          <div className="flex items-center gap-2">
            <Code className="w-3.5 h-3.5 text-cyan-300" />
            <span className="text-xs font-bold text-slate-200">C++ Sketch</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 overflow-auto relative">
          <textarea
            ...
            className="w-full h-full p-4 arduino-code select-text overflow-auto font-mono text-sm leading-relaxed border-none focus:outline-none resize-none bg-slate-950 text-slate-300"
            spellCheck="false"
          />
        </div>
      </div>

      {/* Box 2: Serial Monitor */}
      <div className="glass-panel flex flex-col h-[370px] overflow-hidden flex-shrink-0">
        <div className="flex items-center justify-between border-b border-white/5 bg-slate-950/20 px-4 py-2 flex-shrink-0">
          <div className="flex items-center gap-2">
            <Terminal className="w-3.5 h-3.5 text-amber-300" />
            <span className="text-xs font-bold text-slate-200">Serial Monitor</span>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="flex-1 flex flex-col bg-slate-950 p-3 font-mono text-xs text-amber-500 overflow-hidden">
          ...
          {serialLogs.some(l => l.includes('ERROR') || l.includes('CRITICAL')) && (
            <div className="p-2 bg-red-950/20 border border-red-500/20 rounded-lg text-[10px] text-red-300 flex items-center justify-between mb-2 flex-shrink-0">
              ...
              <button
                ...
                className="px-2 py-0.5 rounded bg-red-500/30 hover:bg-red-500/40 border border-red-500/40 text-red-200 transition-all font-bold font-sans uppercase text-[9px] flex-shrink-0"
              >
                Diagnose
              </button>
            </div>
          ...
          <form onSubmit={handleSerialSubmit} className="flex gap-2 pt-2 border-t border-white/5 flex-shrink-0">
            <input
              type="text"
